...
Before we dive into TrueSkill, let's talk about its predecessor: **Elo**.
...
In 2007, Microsoft Research unveiled **TrueSkill** to solve these problems for Xbox Live matchmaking. It would go on to power Halo, Gears of War, and countless other games.
...
TrueSkill is a **Bayesian** rating system. That's a fancy way of saying it keeps track of both **what we believe** and **how confident we are** in that belief.

In traditional Elo, a player has one number: their rating. In TrueSkill, every player (or pendant) has *two* numbers:

| Symbol | Name | Meaning |
|--------|------|---------|
| **μ** (mu) | Mean | Our best guess at the true skill |
| **σ** (sigma) | Standard deviation | How uncertain we are about that guess |

When you first add a pendant to the system, we know nothing about it:

```python
# Default TrueSkill values
μ = 25.0   # Middle of the skill range
σ = 8.333  # Very uncertain (σ = μ/3)
```

That high σ = 8.333 is TrueSkill's way of saying: "I have no idea how good this pendant is. It could be anywhere from terrible to amazing."
...
1. Takes the prior beliefs (μ, σ) for both items
2. Incorporates the observed outcome
3. Returns updated posterior beliefs

Under the hood, it's doing **Gaussian belief propagation**—message passing on a factor graph. But you don't need to understand that to use it effectively.
...
def conservative_score(mu: float, sigma: float) -> float:
    """99.7% confidence lower bound on true skill."""
    return mu - 3.0 * sigma
```
...
graph LR
    subgraph Lucky["Pendant X (Lucky)"]
        X["μ = 29.4, σ = 6.5<br/>Conservative: 29.4 - 19.5 = <b>9.9</b>"]
    end
    
    subgraph Proven["Pendant Y (Proven)"]
        Y["μ = 32.1, σ = 2.1<br/>Conservative: 32.1 - 6.3 = <b>25.8</b>"]
    end
...
flowchart TD
    ...
    Choice -->|"Neither / Bad pair"| Skip["Outcome: S (Skip)"]
...
|---------|---------------|
| **Two numbers (μ, σ)** | Track both skill and confidence |
| **Bayesian updates** | Learn from every comparison |
| **Shrinking σ** | Uncertainty decreases with evidence |
| **Conservative score** | Don't trust lucky newcomers |
| **Draw handling** | "Too close to call" is valid data |
