...
- Items that are obviously far apart in appeal (uninformative)
...
def choose_next_pair_close(pendants):
    # Pick items with similar estimated appeal
...
1. For each pendant, **sample** an appeal value from its posterior distribution
2. Sort all pendants by their sampled appeal
3. Look at adjacent pairs (close in sampled appeal)
4. Pick the best one
...
## The Recent Matches View

The app tracks every comparison in a "Recent Matches" panel, providing transparency into the learning process:

![Recent matches list](./screenshots/recent_matches.png)
*Figure 6: The recent matches panel shows the history of comparisons. Each row displays the two pendants compared, the outcome (Left/Right/Draw/Skip), and when it occurred.*

This serves multiple purposes:

1. **Transparency**: See exactly what the algorithm has learned from
2. **Debugging**: Verify the algorithm is exploring different pairs, not stuck in loops
3. **Cooldown enforcement**: Recently skipped/drawn pairs are excluded for 2 rounds
4. **Progress tracking**: Watch how the algorithm shifts from exploration (new items) to exploitation (refining top ranks)
...
def choose_next_pair(
    ...
    with get_session() as session:
        ...
        for (a, b) in candidates:
            ...
            # Compute E[Δσ] with repeat penalty
