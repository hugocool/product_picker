...
def _get_recent_skips_and_draws(
    session: Session, folder: str, last_n: int = 3
) -> set[Tuple[int, int]]:
    """Get pairs that were skipped or drawn in the last N matches."""
...
def choose_next_pair(
    ...
    Strategy:
    1. Prioritize items with high uncertainty (sigma) - exploration
    2. Prefer matchups with closer mu (harder decisions) - exploitation
    3. Penalize repeated comparisons
    4. Heavily penalize pairs skipped/drawn in the last few matches (temporary cooldown)
    5. But allow them back if they're highly informative (explore/exploit trade-off)

    This implements TrueSkill's information-theoretic approach: pairs with high uncertainty
    and close skill ratings provide the most information gain (reduce uncertainty most).
    ...
    with get_session(folder) as session:
        ...
        recent_skip_draw = _get_recent_skips_and_draws(session, folder, last_n=3)
    ...
    for left in left_pool:
        for right in right_pool:
            ...
            # Skip explicitly excluded pair (immediate repeat prevention)
            if exclude_pair and (a, b) == exclude_pair:
                continue

            repeat = counts.get((a, b), 0)

            # Heavy penalty for recently skipped/drawn pairs, but not permanent exclusion
            # This allows them to come back if they're highly informative
            recent_skip_penalty = 10.0 if (a, b) in recent_skip_draw else 0.0

            # Information score: high when both have high sigma and close mu
            # This is the TrueSkill information gain principle:
            # - High sigma = high uncertainty = more to learn
            # - Close mu = uncertain outcome = maximum information gain
            mu_gap = abs(left.mu - right.mu) + 1e-6
            info = (left.sigma + right.sigma) / mu_gap
            
            # Apply penalties but allow high information pairs to override
            score = info - 0.75 * repeat - recent_skip_penalty
