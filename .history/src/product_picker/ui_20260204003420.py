...
def load_folder_and_first_pair(folder):
    ...
    if isinstance(folder, list):
        if not folder:
            return "", None, None, "❌ No folder selected. Click a folder in the explorer above.", None, None, "", "", pd.DataFrame(), pd.DataFrame(), ""
        folder = folder[0]
    ...
    if not Path(folder_abs).exists():
        return "", None, None, f"❌ Folder does not exist: {folder_abs}", None, None, "", "", pd.DataFrame(), pd.DataFrame(), ""
    
    if not Path(folder_abs).is_dir():
        return "", None, None, f"❌ Not a directory: {folder_abs}", None, None, "", "", pd.DataFrame(), pd.DataFrame(), ""
