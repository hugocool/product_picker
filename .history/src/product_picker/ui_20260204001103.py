...
def render_pair(folder: str, left_id: int, right_id: int) -> Tuple[Image.Image, Image.Image, str, str]:
...
def load_folder_and_first_pair(folder: str):
    ...
    return folder_abs, left_id, right_id, status, left_img, right_img, left_md, right_md, lb, hist, ""
...
def decide_and_advance(folder: str, left_id: Optional[int], right_id: Optional[int], outcome: str):
    ...
    return nL, nR, "Showing next comparison.", left_img, right_img, left_md, right_md, lb, hist, last
...
def create_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="Pendant Chooser") as demo:
        ...
        with gr.Row():
            ...
            load_btn = gr.Button("Load / Rescan", scale=1)
