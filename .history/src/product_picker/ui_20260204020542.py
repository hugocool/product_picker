...
def get_top_3_display(
    folder: str,
) -> Tuple[Optional[Image.Image], str, Optional[Image.Image], str, Optional[Image.Image], str]:
    ...
    for idx in range(3):
        if idx < len(lb):
            ...
            file_path = lb.iloc[idx]["file"]

            # Load image directly from the leaderboard data
            try:
                from pathlib import Path

                img_path = Path(folder) / file_path
                if not img_path.exists():
                    results.extend([None, f"_File not found: {file_path}_"])
                    continue
                ...
                info = (
                    ...
                    f"({lb.iloc[idx]['W']}-{lb.iloc[idx]['D']}-{lb.iloc[idx]['L']})"
                )
                results.extend([img, info])
            except Exception as e:
                results.extend([None, f"_Error: {str(e)}_"])
        else:
...
def load_folder_and_first_pair(folder):
    ...
    if isinstance(folder, list):
        if not folder:
            return (
                ...
                None,
                None,
                "❌ No folder selected. Click a folder in the explorer above.",
                None,
                None,
                "",
                "",
    ...
    if not Path(folder_abs).exists():
        return (
            ...
            None,
            None,
            f"❌ Folder does not exist: {folder_abs}",
            None,
            None,
            "",
            "",
...
def decide_and_advance(folder: str, left_id: Optional[int], right_id: Optional[int], outcome: str):
    ...
    if left_id is None or right_id is None:
        top1_img, top1_info, top2_img, top2_info, top3_img, top3_info = get_top_3_display(
            folder_abs
        )
        return (
            None,
    ...
    # Get next pair
    # The matching algorithm now checks recent skip/draw history automatically
    # and applies temporary penalties while still allowing highly informative pairs
    nxt = choose_next_pair(folder_abs)
    if nxt is None:
        return (
            None,
            None,
            "Done (or not enough images).",
            None,
            None,
            "",
            "",
...
def create_ui() -> gr.Blocks:
    ...
    # Use home directory as the root for navigation (allows navigating anywhere)
    explorer_root = str(Path.home())
    ...
    with gr.Blocks(title="Pendant Chooser") as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                selected_folder = gr.Textbox(
                    ...
                    value=last_folder or "",
                    placeholder="Click a folder in the explorer or type a path...",
                    interactive=True,
                )
                ...
                gr.Markdown(
                    ...
                    "2. The path will appear above (or type a path directly)\n"
                    "3. Click 'Load / Rescan'"
                )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                gr.Markdown("### 🥇 #1")
                top1_img = gr.Image(type="pil", label="", show_label=False, height=400)
                top1_info = gr.Markdown("")

            with gr.Column(scale=1):
                gr.Markdown("### 🥈 #2")
                top2_img = gr.Image(type="pil", label="", show_label=False, height=400)
                top2_info = gr.Markdown("")

            with gr.Column(scale=1):
                gr.Markdown("### 🥉 #3")
                top3_img = gr.Image(type="pil", label="", show_label=False, height=400)
        ...
        def update_selected_folder(file_path):
            ...
            if not file_path:
                return gr.update()  # Don't clear the textbox if nothing selected
            if isinstance(file_path, list):
                if not file_path:
                    return gr.update()
                file_path = file_path[0]

            # FileExplorer returns paths relative to root_dir
            # Join with the explorer root to get absolute path
            path = Path(explorer_root) / file_path
            path = path.resolve()
