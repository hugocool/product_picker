...
def load_folder_and_first_pair(folder):
    ...
    # Handle FileExplorer output (can be a list or string)
    if isinstance(folder, list):
        if not folder:
            return "", None, None, "No folder selected", None, None, "", "", pd.DataFrame(), pd.DataFrame(), ""
        folder = folder[0]
        ...
        with gr.Row():
            with gr.Column(scale=4):
                folder_picker = gr.FileExplorer(
                    label="📁 Select folder containing pendant images",
            ...
            with gr.Column(scale=2):
                ...
                gr.Markdown("**Tip:** Click on a folder name in the explorer, then click 'Load / Rescan'")
