...
def open_folder_picker(current_path: str = "") -> str:
    ...
    folder = filedialog.askdirectory(
        title="Select folder containing pendant images",
...
def create_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="Pendant Chooser") as demo:
        ...
        # Folder picker modal
        with gr.Row(visible=False) as folder_picker_row:
            with gr.Column(scale=1):
                gr.Markdown("### 📂 Quick Access")
                common_folders_radio = gr.Radio(
                    choices=[f"{name}: {path}" for name, path in get_common_folders()],
                ...
                if recent_folders:
                    gr.Markdown("### 🕐 Recent Folders")
                    recent_folders_radio = gr.Radio(
                        choices=recent_folders,
                        label="Recently Used",
                        interactive=True,
                    )
                else:
                    recent_folders_radio = gr.Radio(visible=False)
                
                with gr.Row():
                    select_folder_btn = gr.Button("✓ Select", variant="primary")
                    cancel_folder_btn = gr.Button("✗ Cancel")
        ...
        def hide_folder_picker():
            """Hide the folder picker."""
            return gr.Row(visible=False)
        
        def select_from_common(choice):
            """Extract path from common folder choice."""
            if not choice:
                return "", gr.Row(visible=False)
            # Format is "Name: /path/to/folder"
            path = choice.split(": ", 1)[1] if ": " in choice else choice
            return path, gr.Row(visible=False)
        
        def select_from_recent(choice):
            """Select from recent folders."""
            if not choice:
                return "", gr.Row(visible=False)
            return choice, gr.Row(visible=False)
        
        browse_btn.click(
            show_folder_picker,
            outputs=[folder_picker_row],
        )
        ...
        common_folders_radio.change(
            select_from_common,
            inputs=[common_folders_radio],
            outputs=[folder_tb, folder_picker_row],
        )
        
        if get_recent_folders():
            recent_folders_radio.change(
                select_from_recent,
                inputs=[recent_folders_radio],
                outputs=[folder_tb, folder_picker_row],
            )
